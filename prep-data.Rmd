...
# ipums_raw2 <- read_csv("data/ipums_acs-2018-1yr_ny.csv.gz") %>%
#   rename_with(str_to_lower)
```
...
ipums_clean <- ipums_raw %>% 
  ...
  mutate(
    ...
    ui_benefits_month_reg = case_when(
      inc_wages_qtr <= 1600 ~ 0,
      inc_wages_qtr > 1600 ~ if_else(inc_wages_qtr/26 < 143, 143, inc_wages_qtr/26),
      TRUE ~ 0
    ) %>%
      if_else(. > 504, 504, .) * 4,
